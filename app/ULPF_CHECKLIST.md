...
| Applications | ✅ | `app_logger` (the stock Logback/Log4j2 layout), `aria_automation_springboot`, `vrlcm_classic_logback`, `spring_actuator_access`, `jvm_gc` |
| Databases | ✅ | `postgres_log`, `postgres_native_log` — including the `%u@%d` log_line_prefix, which records the role and database as attributes |
...
| Containers | ✅ | `kubelet_glog`, `k8s_access`, `coredns`, `envoy_access` (ESXi's syslog-wrapped form) and `envoy_upstream_access` (the service-mesh sidecar's own format) |
...
| IoT devices | ✅ | Modbus gateway, MQTT broker and UPS/PDU samples all parsed by `pri_syslog` with their key=value payloads extracted (8, 7 and 2 attributes) — `batch/test_source_coverage.py` |
| Other hardware / software | ✅ | `generic_fallback` normalizes structurally rather than rejecting |

**53 detectors registered**, 8 of them JSON-aware. Verified: `len(DETECTORS) == 53`.

LEEF extension fields are split on an **inferred** delimiter when the header does not declare one. The spec default is a tab, so the pipe-delimited form Check Point and several QRadar integrations actually send used to yield a single field in which the first key swallowed the rest of the line — 13 attributes now where there was 1.

Coverage is asserted, not asserted-about: `python -m batch.test_source_coverage` parses
`LA/sample_logs.{txt,xml,csv}` — 38 lines spanning every category and format above — and
fails if any of them reaches `generic_fallback`. Current result: **38/38 claimed by a named
detector**, 84 checks passed.
...
| XML | ✅ | `structured_readers._iter_xml` — Windows Event XML resolves `Computer` to the hostname and keeps `<Data Name=..>` values |
| CSV | ✅ | `structured_readers._iter_csv` |
| CEF | ✅ | `detect_cef` — Palo Alto sample parsed, `src`/`dst`/`dpt` extracted; `rt=` normalized to ISO 8601 (it was previously stored verbatim, which left `day` null and dropped every CEF event out of date-range queries) |
...
### 1.3 Measured parsing accuracy

Measured over 150,037 logical records reassembled from all 9,280 files in the
corpus — logical *records*, because every ingestion path
(`batch/parse_logs.py`, `realtime/dnif_api.py`, `structured_readers`) runs
`multiline.reassemble()` first, and a Windows Security event body line or a
Java stack frame is a continuation of the record above it rather than an
unparsed record of its own.

| Measure | Result |
|---|---|
| Records claimed by a **named** detector | **149,910 / 150,037 = 99.9%** |
| Excluding binary archives read as text | **149,910 / 149,946 = 99.98%** |
| Records reaching `generic_fallback` | 127 (91 of them binary, 36 text) |
| Raw event preserved verbatim | 100% — requirement (a) |

The 36 unmatched text records are not log records in any meaningful sense:
shell trace output (`+ sleep 5s`), a bare `[`, one-off systemd notices. There
is no parser to write for them.

**Two things this number is not.** It is not evidence of 100% accuracy on an
unseen estate — these detectors were written against this corpus, so measuring
them on it is partly circular, and no format-detection framework is 100% on
arbitrary future input. And it is a *claim* rate, not a field-level
correctness audit; `batch/test_custom_formats.py` (50 checks) is what asserts
the extracted fields are right.

What carries an unseen source is the fallback plus parser synthesis, below.

### 1.4 Behaviour on sources nobody has written a detector for

This is requirement (e) — plug-and-play onboarding — and it rests on the
fallback path, so it is worth stating what that path does and does not do.

| Property | Status |
|---|---|
| Record is never dropped, raw bytes preserved | ✅ the fallback always returns a record |
| Timestamp, severity, key=value pairs recovered | ✅ generically |
| Hostname recovered **without site-specific configuration** | ✅ from an explicit `host=`/`dvchost=` field, or positionally from the syslog host slot |
| Emitting process recovered | ✅ from syslog's `proc[pid]:` slot |
| Binary content (rotated archives, core dumps) | ✅ flagged `binary_content`, not guessed at — and still stored losslessly |
| Deep vendor-specific field extraction | ❌ by definition; `batch/synth_parser.py` generates a detector from samples in minutes — requirement (i) |

The fallback used to search only for `*.corp.example`, `MD*`, `md-*` and
`*.apps.svc.cluster.local` — the naming of the one estate the corpus came
from — so on any other network it returned no hostname at all. That is fixed:
the rules are now positional and hold anywhere, with site naming demoted to
optional configuration (`ULPF_SITE_DOMAINS`, `ULPF_HOST_PATTERN`).
`batch/test_source_coverage.py` asserts this using hostnames from estates that
...
The OCSF column is no longer only a document. `analytics_pipeline/ocsf.py`
emits **OCSF 1.3.0** for three classes — Authentication (3002), Network
Activity (4001) and Application Lifecycle (1008) — available as an export
format and measurable at `GET /api/analytics/ocsf/classes`.

Stated honestly, because the distribution is the interesting part: on our
VMware/Aria corpus **95.1%** of events land in 1008, which is correct — they
genuinely are application lifecycle events, not security events. On a
heterogeneous perimeter sample the split is **73.7% / 15.8% / 10.5%**. An
event that does not clearly belong to 3002 or 4001 goes to 1008 rather than
being forced into a class whose required fields would have to be invented; a
wrong `class_uid` is worse than a generic one, because a SIEM routes on it.
...
| OCSF JSON | OCSF 1.3.0 — 3 event classes implemented, see (c) |
